...
export default function TakeQuizPage() {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      ...
      try {
        const response = await fetch(
          `/api/get-quiz-question?courseId=${courseId}&questionNumber=${questionNumber}&studentId=${studentId}`
        );
    ...
    fetchQuestion();
  }, [courseId, questionNumber, studentId]);
  ...
  const handleNextQuestion = async () => {
    if (questionNumber < totalQuestions) {
      ...
      router.push(`/quiz/complete?courseId=${courseId}&studentId=${studentId}`);
    }
  };
